...
from nn.no_swarm_perceptron_qpso_layer_e import perceptron 
...
def main(data_name, training_type, algorithm, max_iter_pass, n_particulas_pass):
    ...
    for i in range(n_training):
        if (i==0):
            gBest.append(nn.train(X_train, y_train, X_test, y_test, beta, n_particulas, max_iter, 100, 100, i))
        else:
            print(">>>>>>>>>> Parameters from previous training <<<<<<<<<<<<")
            gBest.append(nn.train(X_train, y_train, X_test, y_test, beta, n_particulas, max_iter, hidden_error, output_error, i, *gBest[best_test_val_pred]))

        hidden_error = nn.error_hidden
        output_error = nn.error_output
        ...
        if (cost_test[i] <= best_cost_test and cost_val[i] <= best_cost_val):
...
if __name__ == '__main__':
    datasets = ["iris", "wine", "circle", "breast"]
    training_type = 'no_exchange_layers'
    algorithm = 'qdpso'

    iterations_list = [20, 40, 60, 80, 100, 500, 1000]
