...
from nn.one_swarm_perceptron_qpso import perceptron 
...
def main(data_name, training_type, algorithm, max_iter_pass, n_particulas_pass):
    ...
    for i in range(n_training):        
        gBest.append(nn.train(X_train, y_train, X_test, y_test, beta, n_particulas, max_iter, i))
        gBest_value.append(nn.g_best_value)
        metric.append(nn.g_best_value_iter)
    ...
    np.save(f"./output/{training_type}/{data_name}/{max_iter}/{n_particulas}/{algorithm}_{data_name}_metric_113_{n_particulas}_{max_iter}_{X_input}.npy", np.array(metric))
...
if __name__ == '__main__':    
    ...
    training_type = 'one_individual_layers'
    algorithm = 'qdpso'
