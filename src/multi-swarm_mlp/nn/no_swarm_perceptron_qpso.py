...
class perceptron(object):
    ...
    def log_hidden(self, s):
        ...
        if (cost_pred_hidden < self.error_hidden):
            ...
            self.param[:self.d_hidden] = s.gbest
    ...
    def train(self, X, y, X_test, y_test, beta, n_particulas, max_iter, i):
        ...
        s_output.update(callback=self.log_output, interval=1)        
        end_time_output = default_timer()
        self.timings.append(("total_output", start_time_output, end_time_output))
        ...
        s_hidden.update(callback=self.log_hidden, interval=1)
        end_time_hidden = default_timer()
        self.timings.append(("total_hidden", start_time_hidden, end_time_hidden))
