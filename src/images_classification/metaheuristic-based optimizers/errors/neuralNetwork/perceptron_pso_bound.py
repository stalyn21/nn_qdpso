...
class perceptron_pso_bound(object):
    ...
    def f(self, x):
        particles = x.shape[0]
        j = [self.backware(x[i]) for i in range(particles)]
        #self.error_loss.append(j)
        return j
    ...
    def train(self, X, y, opt, n_particulas, max_iter):
        print("training nn with PSO")
        self.X = X
        self.y = self.one_hot_encode(y)
        
        #initalize parameters for pso 
        max_bound = 1 * np.ones(self.d)
        min_bound = - max_bound
        bounds = (min_bound, max_bound)
        NParticle = n_particulas
        MaxIters = max_iter

        optimizer = ps.single.GlobalBestPSO(n_particles=NParticle, dimensions=self.d, options=opt, bounds=bounds)
        self.best_cost, best_pos = optimizer.optimize(self.f, iters=MaxIters)

        self.h_cost = optimizer.cost_history
        self.h_pos = optimizer.pos_history
        self.avg_best_value = optimizer.mean_pbest_history
